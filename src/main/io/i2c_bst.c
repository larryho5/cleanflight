...
static bool bstSlaveProcessFeedbackCommand(uint8_t bstRequest)
{
	...
	switch(bstRequest) {
	    ...
	    case BST_FC_FILTERS:
			...
			bstWrite8(masterConfig.profile[0].pidProfile.gyro_cut_hz);
			bstWrite8(masterConfig.profile[0].pidProfile.dterm_cut_hz);
			bstWrite8(masterConfig.profile[0].pidProfile.pterm_cut_hz);
			break;
		default:
...
static bool bstSlaveProcessWriteCommand(uint8_t bstWriteCommand)
{
	...
	switch(bstWriteCommand) {
	    ...
	    case BST_SET_FC_FILTERS:
			...
			masterConfig.profile[0].pidProfile.gyro_cut_hz = bstRead8();
			masterConfig.profile[0].pidProfile.dterm_cut_hz = bstRead8();
			masterConfig.profile[0].pidProfile.pterm_cut_hz = bstRead8();
			break;
